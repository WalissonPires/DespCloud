	            ...
	            if (prop.indexOf('[') === -1) { //is object prop
	                ...
	                if (j+1 >= dataNames.length) { //is last prop
	                	...
	                	if (dataIndex === null) { //is primitive value
					   		...
					   		root[prop] = options.formatterValue(data.value, data.name);
							...
							root[dataIndex][prop] = options.formatterValue(data.value, data.name);
							dataIndex = null;
	                    }
	                ...
	                if (j+1 >= dataNames.length) 
	                    root.push(options.formatterValue(data.value, j > 0 ? data.name : ''));
	                else {
